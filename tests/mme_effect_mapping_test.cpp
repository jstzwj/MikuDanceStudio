...
int main() {
    ...
    expect({{"C:\\EFFECTS\\RAY_CONTROLLER.PMX", "legacy.fx"}}, &controller, &ray, -1,
           "annotation keys only match basename, never full path");
    ...
    expect({{controller.filename(), "unexpected.fx"}, {"*", "hide"}},
           &controller, &ray, 1, "full path must not steal the fallback row");
