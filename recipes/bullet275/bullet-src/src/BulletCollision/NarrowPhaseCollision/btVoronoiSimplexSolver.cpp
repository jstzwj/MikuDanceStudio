...
bool	btVoronoiSimplexSolver::updateClosestVectorAndPoints()
{
	...
	if (m_needsUpdate)
	{
		...
		case 2:
			...
			// VC9 orders (MMD 0x4EEB7E): the projection dot is grouped
			// (dx*(-x0) + dz*(-z0)) + dy*(-y0), dotVV is
			// (dx*dx + dz*dz) + dy*dy, and t is an SSE division.
					const btVector3& from = m_simplexVectorW[0];
					const btVector3& to = m_simplexVectorW[1];

					const btScalar dx = to.x() - from.x();
					const btScalar dy = to.y() - from.y();
					const btScalar dz = to.z() - from.z();
					btScalar t = (dx*(-from.x()) + dz*(-from.z()))
						+ dy*(-from.y());

					if (t > 0) {
						btScalar dotVV = (dx*dx + dz*dz) + dy*dy;
						if (dotVV > t) {
							t = t / dotVV;
					...
					m_cachedP1 = btVector3(
						(m_simplexPointsP[1].x()-m_simplexPointsP[0].x())*t + m_simplexPointsP[0].x(),
						(m_simplexPointsP[1].y()-m_simplexPointsP[0].y())*t + m_simplexPointsP[0].y(),
						(m_simplexPointsP[1].z()-m_simplexPointsP[0].z())*t + m_simplexPointsP[0].z());
					m_cachedP2 = btVector3(
						(m_simplexPointsQ[1].x()-m_simplexPointsQ[0].x())*t + m_simplexPointsQ[0].x(),
						(m_simplexPointsQ[1].y()-m_simplexPointsQ[0].y())*t + m_simplexPointsQ[0].y(),
						(m_simplexPointsQ[1].z()-m_simplexPointsQ[0].z())*t + m_simplexPointsQ[0].z());
...
bool	btVoronoiSimplexSolver::closestPtPointTriangle(const btVector3& p, const btVector3& a, const btVector3& b, const btVector3& c,btSubSimplexClosestResult& result)
{
    ...
    btVector3 ac = c - a;
    btVector3 ap = p - a;
    // VC9 (MMD 0x4EDC90) evaluates every dot in this function with the
    // grouping (z*z + y*y) + x*x; replicate it for d1..d6.
    btScalar d1 = (ab.z()*ap.z() + ab.y()*ap.y()) + ab.x()*ap.x();
    btScalar d2 = (ac.z()*ap.z() + ac.y()*ap.y()) + ac.x()*ap.x();
    ...
    btVector3 bp = p - b;
    btScalar d3 = (ab.z()*bp.z() + ab.y()*bp.y()) + ab.x()*bp.x();
    btScalar d4 = (ac.z()*bp.z() + ac.y()*bp.y()) + ac.x()*bp.x();
    ...
    btVector3 cp = p - c;
    btScalar d5 = (ab.z()*cp.z() + ab.y()*cp.y()) + ab.x()*cp.x();
    btScalar d6 = (ac.z()*cp.z() + ac.y()*cp.y()) + ac.x()*cp.x();
...
int btVoronoiSimplexSolver::pointOutsideOfPlane(const btVector3& p, const btVector3& a, const btVector3& b, const btVector3& c, const btVector3& d)
{
    ...
    // VC9 (MMD 0x4EE360) groups both plane dots as (z*z + x*x) + y*y.
    btScalar signp = ((p.z()-a.z())*normal.z() + (p.x()-a.x())*normal.x())
        + (p.y()-a.y())*normal.y(); // [AP AB AC]
    btScalar signd = ((d.z()-a.z())*normal.z() + (d.x()-a.x())*normal.x())
        + (d.y()-a.y())*normal.y(); // [AD AB AC]
...
bool	btVoronoiSimplexSolver::closestPtPointTetrahedron(const btVector3& p, const btVector3& a, const btVector3& b, const btVector3& c, const btVector3& d, btSubSimplexClosestResult& finalResult)
{
   ...
   if (!pointOutsideABC  && !pointOutsideACD && !pointOutsideADB && !pointOutsideBDC)
	...
	if (pointOutsideABC) 
	{
        ...
        btScalar sqDist = ((q.z()-p.z())*(q.z()-p.z()) + (q.y()-p.y())*(q.y()-p.y()))
			+ (q.x()-p.x())*(q.x()-p.x());
